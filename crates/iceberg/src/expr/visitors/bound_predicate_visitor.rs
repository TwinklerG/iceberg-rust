...
mod tests {
    ...
    #[test]
    fn test_transform_term_eq() {
        use crate::expr::TransformTerm;

        let predicate = TransformTerm::bucket("a", 10).equal_to(Datum::int(3));
        let bound_predicate = predicate.bind(create_test_schema(), false).unwrap();
    ...
    #[test]
    fn test_transform_term_in() {
        use crate::expr::TransformTerm;

        let predicate =
            TransformTerm::bucket("a", 10).is_in([Datum::int(1), Datum::int(2), Datum::int(3)]);
        let bound_predicate = predicate.bind(create_test_schema(), false).unwrap();
